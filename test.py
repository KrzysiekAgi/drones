...
def az_elev2(fi1, fi2, L1, L2):
    '''source: https://en.wikipedia.org/wiki/Azimuth
    	https://pl.wikipedia.org/wiki/Ortodroma
    fi1 - site's latitude, fi2 - drone's latitude, L2 - dron's longitude, L1 - site's longitude being equal to 0 (?)'''
    az = m.degrees(m.atan(m.sin(L2)/(m.cos(fi1)*m.tan(fi2)-m.sin(fi1)*m.cos(L2) ) ) )
    #D - ortodroma
    D = m.degrees(m.acos(m.sin(fi1)*m.sin(fi2)+m.cos(fi1)*m.cos(fi2)*m.cos(L2-L1)))
    return (az, D)
